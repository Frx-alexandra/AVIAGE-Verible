...
VERILOG_REGISTER_LINT_RULE(ModificationHistoryCheckRule);

static constexpr std::string_view kRequiredNotice = "MODIFICATION HISTORY";
static constexpr std::string_view kMessage =
    "File must contain \"MODIFICATION HISTORY\" in the comments before any "
    "module definition.";

const LintRuleDescriptor &ModificationHistoryCheckRule::GetDescriptor() {
  static const LintRuleDescriptor d{
      .name = "modification-history-check",
      .topic = "copyright",
      .desc =
          "Checks that every file contains the text \"MODIFICATION HISTORY\" "
          "in the comments before any module definition. This is typically "
          "required for tracking changes to proprietary or confidential source "
          "code files.",
  };
  return d;
}

void ModificationHistoryCheckRule::Lint(const TextStructureView &text_structure,
                                        std::string_view) {
  std::string_view contents = text_structure.Contents();

  // Find the position of MODIFICATION HISTORY
...
absl::Status ModificationHistoryCheckRule::Configure(
    std::string_view configuration) {
  // No configuration needed for this rule
  return absl::OkStatus();
}

LintRuleStatus ModificationHistoryCheckRule::Report() const {
